...
include_once('inc/config.php');
  /*  if(count($_POST)>0){
    ...
    if (move_uploaded_file($_FILES['photo']['tmp_name'], $path)) {
        ...
        $sql = "INSERT INTO product (product_name, description, category_id, price, stock, image) 
                VALUES ('$pname', '$description', '$category', '$price', '$stock', '$path')";
...
if(count($_POST)>0){
    ...
    if (move_uploaded_file($_FILES['photo']['tmp_name'], $path)) {
        ...
        $sql = "INSERT INTO product (cmn_id, product_name,  description, category_id, feature_id, pstatus, price, cutoff_price, stock, image) 
                VALUES ('$cmn_id', '$pname', '$description', '$category', '$feature', '$status', '$price', '$cprice', '$stock', '$path')";
	...
	if ($con->query($sql) === TRUE) {
		...
		if(move_uploaded_file($_FILES["file"]["tmp_name"][$i],'upload/'.$image)){
			...
			// Image db insert sql 
			$insert = "INSERT into files(file_name,uploaded_on,status) values('$image',now(),'$cmn_id')";
